    ...
    if os.path.exists(DB_FILE):
        try:
            with open(DB_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
                for name in data:
                    if 'RC' not in data[name]: data[name]['RC'] = 0.08
                return data
        except:
            pass
    return {}
...
with tab_input:
    ...
    if st.button("📈 분석 실행"):
        ...
        if final_df.empty:
            ...
            total_wmt, total_dmt, total_ndmt = final_df['WMT'].sum(), final_df['DMT'].sum(), final_df['NDMT'].sum()
...
with tab_setting:
    ...
    def add_smelter():
        ...
        # [중요] 이미 처리가 되어 입력창이 비어있다면 함수 종료 (중복 실행 방지)
        if not name_to_add:
            return
        ...
        if name_to_add in st.session_state['smelters']:
            ...
            # [중요] 성공 후 입력창을 비워줌으로써 버튼 중복 클릭 에러 방지
    ...
    with col_add1:
        st.text_input(
            ...
            on_change=add_smelter  # 엔터 입력 지원
        )
    with col_add2:
        st.write(" ") 
        # 버튼 클릭 지원
        if st.button("🚀 추가", use_container_width=True, on_click=add_smelter):
            # 알림(toast)을 보여주기 위해 여기서는 st.rerun()을 사용하지 않습니다.
            # 버튼 클릭 시 on_click이 실행된 후 Streamlit이 자동으로 화면을 갱신합니다.
            pass
    ...
    if smelter_names:
        ...
        with st.form("smelter_edit_form"):
            ...
            with c3:
                ...
                # data_editor는 form 안에서 key가 고정되어야 정상 작동함
            ...
            if submit_btn:
                ...
                save_smelters(st.session_state['smelters']) # JSON 파일 저장 함수 호출
                st.success(f"✅ '{edit_name}' 조건이 성공적으로 저장되었습니다.")
                st.rerun() # 변경사항 즉시 반영을 위한 페이지 새로고침
        ...
        if delete_btn:
            # 현재 등록된 제련소가 1개보다 많을 때만 삭제 허용
            if len(st.session_state['smelters']) > 1:
                del st.session_state['smelters'][selected_smelter]
                save_smelters(st.session_state['smelters'])
                st.rerun()
            else:
                # 1개만 남았을 때는 삭제하지 않고 경고 메시지 출력
